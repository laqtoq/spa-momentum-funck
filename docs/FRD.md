...
| **Version** | 1.5 (draft for human editing; change log in Section 11) |
| **Date** | August 2026 |
...
**Layer 1 — Live stream (seconds).** A Finnhub free-tier WebSocket delivers real-time trades for the 20 watchlist names plus SPY (free plan allows up to 50 symbols per connection). From this stream the application builds, client-side: the live last price per name, 1-minute and 5-minute bars, the running session VWAP (Σ price×volume ÷ Σ volume over streamed trades), the E8 micro-trigger state, and the position monitor's distance-to-stop/target. Finnhub REST is deliberately **not** used (CORS unreliability from browsers); only its WebSocket is.

**Layer 2 — Bars on demand (minutes).** Twelve Data REST (CORS-friendly, 8 req/min, 800 credits/day) supplies 1-hour bars for structure criteria, 5-minute history to seed relative-volume baselines (the same-slot averages over 20 sessions, fetched once per name per day and cached) and to backfill stream gaps, and historical intraday bars for Module B. Daily budget check: 20 names × (1 structure refresh + 1 baseline seed) + drill-down and simulator traffic fits comfortably inside 800 credits with the FR-A5 throttle queue.

**Layer 3 — Reference and context (hours to days).** FMP batch quote (overview table, 50/200-DMA, regime inputs, earnings calendar); quantmod/Yahoo in R for the monthly screen; newsdata.io / NewsAPI / Riskline for LLM context; OpenRouter for the desk note.

### 6.2 Source table

| Source | Auth | Free-tier limits | Used for |
|---|---|---|---|
| Finnhub WebSocket (`wss://ws.finnhub.io`) | API key (token param) | Free plan; up to 50 symbols per connection; REST budget separate (60/min, unused) | Layer 1: real-time trades → live prices, client-side 1m/5m bars, session VWAP, micro-trigger, position monitor |
| Twelve Data (`/time_series`) | API key | 8 req/min, 800 credits/day, US symbols | Layer 2: 1h structure bars, 5m baseline/backfill, historical intraday for Module B, one daily-bar call for the SPY 20-DMA |
| Alpaca Market Data | API key pair | Free: real-time IEX feed + 15-min-delayed SIP; historical minute bars | Documented **alternative** stream/bars provider (adapter interface, NFR-4); REST CORS from browsers unverified, so not primary |
...
| Financial Modeling Prep (`/stable/batch-quote`, earnings calendar) | API key | 250 req/day | Layer 3: overview table, 50/200-DMA, VIX/SPY regime inputs, next earnings dates (blackout logic) |
...
| ID | Requirement |
|---|---|
| FR-A1 | On page load, fetch live quotes for all watchlist tickers plus SPY and VIX via FMP batch calls, plus one Twelve Data daily-bar call for SPY to compute its 20-DMA (the batch quote carries only 50/200-DMA), and render the overview table and regime banner from at most three REST requests. |
...
| FR-A7 | On load, the application determines the next earnings date for each watchlist name and merges configured macro dates from `key_dates.json`. |
...
| FR-A15 | On load, the application opens the Finnhub WebSocket for all watchlist names plus SPY, aggregates trades into client-side 1m/5m bars and the running session VWAP, and displays a stream-status indicator (CONNECTED / DEGRADED / DOWN) with the age of the last tick; automatic reconnection with exponential backoff. |
