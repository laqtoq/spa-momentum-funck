...
| **Author** | Sven-Michael Funck — Finance track |
| **Version** | 1.8 (draft for human editing; change log in Section 11) |
| **Date** | August 2026 |
...
Before quantitative screening, a funnel of *attention signals* surfaces candidates that pure price screens can lag on. The funnel runs **at screening time, on the analyst's machine** — never in the browser and never as a trade trigger. The 2026-08-04 run was executed in **Python** (yfinance for prices, keyless); an R port for the course-facing repository is a fast follow and is tracked as an open item, not a completed one. Its output is a candidate list that still must pass every filter in Section 4.1.

| Source | Access | What it contributes | Constraints |
|---|---|---|---|
| ~~FMP biggest gainers / most actives~~ | REST, API key (free tier) | *Assessed, not used.* The 2026-08-04 free-tier spike found FMP's free tier symbol-restricted for post-Aug-2025 keys (compromise C9), so a market-attention funnel on it is not viable without a paid tier | Superseded; retained here as an assessed-and-rejected source |
...
| Finnhub REST (`/calendar/earnings`) | API key (token param) | Free plan, 60 req/min; bulk calendar capped at 1500 rows (dropping near-term dates), so queried per symbol; **WebSocket unusable: free tier streams only a popular-symbol subset (C9)** | Layer 3: next earnings dates for the blackout logic (20 per-symbol calls); browser CORS verified 2026-08-04 |
...
| FR-A7 | On load, the application determines the next earnings date for each watchlist name via per-symbol Finnhub REST calendar calls (20 calls inside the 60/min budget; the bulk calendar caps responses at 1500 rows nearest the end date, silently dropping near-term dates — verified 2026-08-04). Names absent from the calendar fail safe to "unknown" and are badged per Section 4.6; configured macro dates from `key_dates.json` are merged in. |
