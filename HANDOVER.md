...
**Spike v1 ran 2026-08-04 (live market) and FAILED as designed — fallbacks activated, FRD amended to v1.6 (see C9):** Finnhub free WS streams only a popular-symbol subset (AAPL/TSLA yes, SPY/MU/DELL silent, no error frames); FMP free tier for post-Aug-2025 keys is symbol-restricted (SPY/^VIX OK, watchlist names 402) with batch + legacy `v3` endpoints dead. Verified working: Twelve Data fully serves watchlist names (1h/5m/60d/`/quote` batch); Finnhub REST `/calendar/earnings` works from the browser (CORS fine — v1.5's contrary assumption was wrong); FMP `stable/quote` for SPY + ^VIX. Pivot implemented on branch `data-provider-pivot`: Alpaca IEX WS is Layer 1 (adapter + tests done; browser reachability verified, full verification needs the user's Alpaca paper keys in **spike v2** during market hours), Twelve Data carries overview quotes, Finnhub REST carries earnings, FMP regime-only. **Remaining gate before live wiring: spike v2 check 1 (Alpaca auth + watchlist trades) PASS with real keys.**
...
4. Build order remaining: live wiring (FR-A15–A18) → position monitor UI (journal exists in `store.js`) → Module B → Module D bucket board UI → Module C → R port of the screening pipeline (course-facing; Python original in project history) → docs/deck.
...
- [x] Spike v1 run 2026-08-04 → fallbacks activated + FRD amended to v1.6 (C9) · [ ] Spike v2 check 1 (Alpaca) passed with user keys (screenshot for appendix) · [ ] Spike v2 re-run at deployed URL once Pages exists
